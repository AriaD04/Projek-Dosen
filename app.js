...
const GLB_PATHS = {
  ...
  laut2: "sceen_3_laut2_1.glb", // <--- TAMBAHKAN BARIS INI
...
let s3p2_slots = []; // array {key, mesh slot, occupied}
let s3p2_items = []; // draggable item groups (shuffle tray)
let s3p2_correct = 0;
...
let s3p2_gameStarted = false; // true setelah user tekan "Mulai"
let s3p2_timerInterval = null; // interval ID countdown
let s3p2_timeLeft = 10; // detik tersisa
let s3p2_cameraLocked = false; // apakah kamera sedang di-lock
...
function buildScene3() {
  ...
  function s3OnClick(e) {
    if (currentScene !== 2 || s3_layer !== 1) return;
    const rect = renderer.domElement.getBoundingClientRect();
...
function s3p2_onDown(e) {
  if (currentScene !== 2 || s3_layer !== 2) return;
